...
function isValidDate(iY, iM, iD) {   
   if (iY>2200 || iY<1900 || !checkNumber(iY)){   
            alert("输入身份证号,年度"+iY+"非法！");   
            return false;   
        }   
   if (iM>12 || iM<=0 || !checkNumber(iM)){   
            alert("输入身份证号,月份"+iM+"非法！");   
            return false;   
        }   
   if (iD>31 || iD<=0 || !checkNumber(iD)){   
...
function isChinaIDCard(StrNo){     
  ...
  if (StrNo.length==18)  {     
    var a,b,c     
    if (!checkNumber(StrNo.substr(0,17))){   
