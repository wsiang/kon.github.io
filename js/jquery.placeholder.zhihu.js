    ...
    return this.each(function () { 
      ...
      var position  = function () { 
        ...
        $.each(box_style, function (i, name) { 
		  ...
		  //因为首页的情况在ie浏览器下偏移了一点，这里就特殊处理一下
		  var parMT = $input.css('margin-top');
		  var parMT_num = 0;
		  if (parMT && parMT.length > 0) {
			  parMT_num = parseInt($input.css('margin-top').replace('px',''));
		  }
		  var subMT = parMT_num + 3 + 'px';
		  $layer.css('margin-top', subMT); 
        }) 
      } 
